   ...
   // const [otp, handlechange] = useState('');
    ...
    const resetpass1 =(e) => {
        ...
        }else if(newpassword!==confirmpassword)
        {
            ...
            Axios.post("http://localhost:3001/resetpass1", {
    ...
    return(
       ...
       <div style={{color:"red",fontSize:"20px",marginTop:"8px"}}>{newpassErr}</div>
        ...
        <input type="text" placeholder="username" onChange={(e)=>{setCustname(e.target.value);}}></input> 
        <center><button onClick={resetpass1}>Reset password</button></center>
       </div>
   );
}
