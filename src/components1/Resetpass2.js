...
function ResetPass2(props) {
   ...
   // const [otp, handlechange] = useState('');
    ...
    const [VAdmin_name,setAdminname] = useState('');
    ...
    const resetpass2 =(e) => {
        ...
        }else if(newpassword!==confirmpassword)
        {
            ...
            Axios.post("http://localhost:3001/resetpass2", {
    ...
    return(
       ...
       <div style={{color:"red",fontSize:"20px",marginTop:"8px"}}>{newpassErr}</div>
        ...
        <center><button onClick={resetpass2}>Reset password</button></center>
       </div>
   );
}
